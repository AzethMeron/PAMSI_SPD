 ...
 * 	protected:
 *  	TypWyjsciowy Algorytm(const TypWejsciowy&,ostream&,const int&) override 
 * 		{}
 ...
 * 	Algorytm:
 * 		const TypWejsciowy& - pojedyńcze wczytane wejście
...
class Tester
{
	...
	protected:
		...
		virtual TypWyjsciowy Algorytm(const TypWejsciowy&,std::ostream&,const int&) = 0;
		virtual void WyswietlWynik(std::ostream&,TypWyjsciowy&,const int&,const double&) = 0;
	public:
...
void Tester<TEMPLATE_ARGS>::Start(bool pauza)
{
	...
	// Przetwarzanie
	int index = 1;
	for(const TypWejsciowy& Wsk : Lista)
	{
		Timer zegarek;
		zegarek.Start();
